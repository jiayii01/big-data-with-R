...
us_accidents_cleaned <- us_accidents_cleaned |> 
  ...
  select(-c(Turning_Loop, Weather_Condition, Distance_Mi))
...
crosstab_Severity_Sunrise_Sunset
...
accidents <- accidents |>
  ...
  select(-c(State, Severity, Wind_Chill_C, Weather_Timestamp, Duration))
...
```{r}
balanced_accident_train <- ovun.sample(Is_Severe~., data = accidents_train,
                                  p = 0.5, seed=seed_num,
                                  method = "both")$data

balanced_accident_train |>
  count(Is_Severe) |>
  mutate(prop = n / sum(n, na.rm = T))

balanced_accident_train <- sdf_copy_to(sc, balanced_accident_train, "balanced_accident_train", overwrite = TRUE)
glimpse(balanced_accident_train)
```
...
Using the graph, we have decided to remove Traffic_Calming and Roundabout from our features as they are statistically insignificant.

### Pipeline, Logistic Regression

```{r}
accidents_train <- accidents_train |>
  select(-c(Traffic_Calming, Roundabout))

numerical_col_names_accidents <- accidents_train |>
  select_if(is.numeric) |> select(-c(Is_Severe)) |> colnames()

cat_col_names_accidents <- accidents_train |>
...
pipeline <- ml_pipeline(sc) |>
  ...
  ft_one_hot_encoder(
    input_cols = c("DayOfWk_indexed", "TimeOfDay_indexed", "Weather_Condition_New_indexed"),
    output_cols = c("DayOfWk_encoded", "TimeOfDay_encoded", "Weather_Condition_New_encoded")
  ) |>
  ft_vector_assembler(
    input_cols = c("stdz_features", "DayOfWk_encoded", "TimeOfDay_encoded", "Weather_Condition_New_encoded"),
...
cv_model <- ml_fit(cv, dataset = accidents_train)

ml_validation_metrics(cv_model) |>
  arrange(desc(areaUnderROC))
# after running, best value of elastic_net_param is 0.25, best value of reg_param is 0.01
# 
# pipeline_path <- "accidents_spark_model"
# 
# # Save PipelineModel to disk
# ml_save(cv_model$best_model, path = pipeline_path, overwrite = TRUE)
```

## Load PipelineModel
```{r}
# model_load <- ml_load(sc, "accidents_spark_model")
# 
# 
predictions <- ml_transform(cv_model, accidents_test)

predictions |>
  count(Is_Severe, prediction)
```
...
```{r}
# get_clustering<-function(scores,s){
# scores<-scale(scores)
# SC<-data.table()
# for (k in 2:12){
#     km<-kmeans(scores,k)
#     SC<-rbind(SC,cbind(k,km$tot.withinss,km$betweenss,km$totss))
# }
# colnames(SC) <- c("n","SCI","SCE","SCT")
# print(SC)
# kmeans_fc<-function(x,k) kmeans(x, k, iter.max=50)
# fviz_nbclust(scores,kmeans_fc, method = "wss",linecolor = "red")%>%print()
# fviz_nbclust(scores,kmeans_fc, method = 'silhouette',linecolor = "blue")%>%print()
# fviz_nbclust(scores,kmeans_fc, method = "gap_stat",linecolor = "black")%>%print()
#   ResOp <- kmeans(scores,s)
# cluster <- ResOp$cluster %>% as.data.table()    
# colnames(cluster) <- c("cluster")
# fviz_cluster(ResOp, data = scores,geom = 'point',repel = TRUE,
#              palette='rainbowcols',ellipse.type = "euclid")%>%print()
# return(cbind(scores,cluster) %>% as.data.table())
# }
# df<-get_clustering(accidents,5)
# 
# 
# cluster <- ml_kmeans(
#   sc,
...
# )
# cluster
# num_iterations <- cluster$numIter
# num_iterations
...
# ml_kme
# 
# cluster_model <- ml_fit(cluster, accidents_train)
# 
# cluster
# 
# accidentsss <- data(pipeline)
# 
# set.seed(123)
# km.out <- kmeans(pipeline, centers = 3, nstart = 20)
# km.out
# 
# 
