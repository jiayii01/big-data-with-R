...
crosstab_Severity_Wind_Direction_New
...
accidents_train_1 <- accidents_train |>
  select(-c(Traffic_Calming, Roundabout, Civil_Twilight))
...
accidents_train_2 <- accidents_train |>
  select(-c(Traffic_Calming, Roundabout, Weather_Condition_New, Civil_Twilight))
...
accidents_train_3 <- accidents_train |>
  select(-c(Traffic_Calming, Roundabout, Weather_Condition_New, Civil_Twilight, DayOfWk))
...
pipeline3 <- ml_pipeline(sc) |>
  ...
  ft_one_hot_encoder(
    input_cols = c("TimeOfDay_indexed"),
    output_cols = c("TimeOfDay_encoded")
  ) |>
  ft_vector_assembler(
    input_cols = c("stdz_features", "TimeOfDay_encoded"),
    output_col = "final_features"
  ) |>
  ml_logistic_regression(
    max_iter = 1000,
